  ...
  public tokenizeLine(line: string, isFirstLine: boolean): HighlightToken[] {
    ...
    if (this.inExtensionTag) {
      ...
      if (closeIdx > -1) {
        ...
        if (content) {
          tokens.push({
            text: content,
            className: `wt-ext-${this.inExtensionTag}`,
          });
        }
        tokens.push({
          text: closeTag,
          className: `wt-exttag wt-ext-${this.inExtensionTag}`,
        });
    ...
    while (pos < line.length) {
      ...
      if (remaining.startsWith("{{{")) {
        ...
        while (
          varPos < remaining.length &&
          remaining[varPos] !== "|" &&
          remaining.slice(varPos, varPos + 3) !== "}}}"
        ) {
          varPos++;
        }
        ...
        if (isComplete) {
          ...
          tokens.push({
            text: open,
            className: "wt-templateparameter-bracket",
          });
          if (name)
            tokens.push({ text: name, className: "wt-templateparameter-name" });
          for (let p = 0; p < params.length; p++) {
            tokens.push({
              text: "|",
              className: "wt-templateparameter-delimiter",
            });
            if (params[p])
              tokens.push({
                text: params[p],
                className: "wt-templateparameter",
              });
          }
        }
        pos += fullLength;
      ...
      if (this.templateDepth > 0) {
        if (remaining.startsWith("{{{")) {
          ...
          if (isComplete) {
            tokens.push({
              text: remaining.slice(0, fullLength),
              className: "wt-template-var",
            });
          } else {
            tokens.push({
              text: open,
            ...
            if (name)
              tokens.push({
                text: name,
                className: "wt-templateparameter-name",
              });
            for (let p = 0; p < params.length; p++) {
              tokens.push({
                text: "|",
      ...
      if (remaining.startsWith("[[")) {
        ...
        while (
          linkPos < remaining.length &&
          remaining[linkPos] !== "|" &&
          remaining.slice(linkPos, linkPos + 2) !== "]]"
        ) {
          linkPos++;
        }
        ...
        if (isComplete) {
          ...
          for (let p = 0; p < params.length; p++) {
            ...
            if (params[p])
              tokens.push({ text: params[p], className: "wt-link-label" });
          }
        }
        pos += fullLength;
        continue;
      ...
      if (remaining.match(/^\[https?:\/\//i)) {
        ...
        if (isComplete) {
          ...
          if (protocolMatch) {
            tokens.push({
              text: protocolMatch[0],
              className: "wt-extlink-protocol",
            });
            tokens.push({
              text: url.slice(protocolMatch[0].length),
              className: "wt-extlink-url",
            });
          } else {
          ...
          if (hasSpace) {
            ...
            if (label)
              tokens.push({ text: label, className: "wt-extlink-label" });
          }
        }
        pos += fullLength;
        continue;
  ...
  private parseToken(remaining: string): HighlightToken | null {
    ...
    if (remaining.startsWith("<")) {
      ...
      if (tagMatch) {
        ...
        const isContentPreserving =
          this.contentPreservingTags.includes(tagName);
          ...
          if (openTagMatch) {
            ...
            if (!isSelfClosing) {
              ...
              if (closeIdx > -1) {
                ...
                return {
                  text: remaining,
                  className: `wt-ext-${tagName}-start`,
                };
              }
            }
          }
    ...
    if (remaining.startsWith("'''''")) {
      ...
      if (closeIdx > -1) {
        return {
          text: remaining.slice(0, closeIdx + 5),
          className: "wt-strong-em",
        };
      }
    }
    ...
    if (remaining.startsWith("'''")) {
      ...
      if (closeIdx > -1) {
        return {
          text: remaining.slice(0, closeIdx + 3),
          className: "wt-strong",
        };
      }
    }
